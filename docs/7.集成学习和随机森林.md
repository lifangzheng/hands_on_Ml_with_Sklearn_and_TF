...
假设你已经训练了一些分类器，每一个都有 80% 的准确率。你可能有了一个Logistic Regression、一个SVM、一个Random Forest，一个 KNN，或许还有更多（见图 7-1）
...
> 如果每一个分类器都尽可能独立，那么集成模型会工作的很好。获得多样分类器的方法之一就是用完全不同的算法。这增加了它们做出不同种类错误的可能性，也提高集成的正确率。
...
一个让分类器能获得多种分类的方法是使用差别很大的训练算法（来训练不同的预测器们），就像刚刚讨论的一样。另一种方法就是对每一个分类器都使用相同的训练算法，但是在不同的训练集上去训练它们。有放回采样被称为装袋（*Bagging*，是 *bootstrap aggregating* 的缩写）。无放回采样称为粘贴（*pasting*）。
...
正如你在图 7-4 上所看到的，分类器可以在不同的 CPU 核或其他的服务器上被并行训练。相似的，预测的过程也可以并行处理。这就是为什么 Bagging 和 Pasting 是如此流行的原因之一：它们的可扩展性很好。
...
对于 Bagging 来说，一些实例可能被一些分类器重复采样，但其他的有可能不会被采样。`BaggingClassifier`默认有放回采样`m`个实例 （`bootstrap=True`），其中`m`是训练集的大小，这意味着平均下来只有63%的训练实例被每个分类器采样，剩下的37%个没有被采样的训练实例就叫做包外（Out-of-Bag）实例。注意对于每一个的分类器它们的 37% 不是相同的。

由于在训练中预测器没有用过这些包外（oob）实例，所以这些实例可用于预测器的评估，而不需要单独的验证集或交叉验证。你可以拿出每一个预测器的 oob 来评估集成分类器本身。

在 sklearn 中，你可以在创建`BaggingClassifier`时设置`oob_score=True`让其结束训练后自动进行oob评估。接下来的代码展示了这个操作。评估结果通过变量`oob_score_`来显示：
...
根据这个 obb 评估，`BaggingClassifier`可以在测试集上达到93.1%的准确率，让我们证实一下：
...
正如我们所讨论的，随机森林是决策树的一种集成，一般通过 bagging 方法（有时是 pasting 方法）进行训练，通常用`max_samples`设置为训练集的大小。不同于建立一个`BaggingClassifier`然后设置 *DecisionTreeClassifier* ，你可以使用`RandomForestClassifier`（对于回归是`RandomForestRegressor`），这更方便而且使用的是优化后的决策树算法。接下来的代码训练了带有 500 个树（每个被限制为 16 叶子结点）的决策森林，使用所有可用的 CPU 核：
...
除了一些例外，`RandomForestClassifier`使用`DecisionTreeClassifier`的所有超参数（决定树怎么生长），把`BaggingClassifier`的超参数加起来来控制集成本身。

随机森林算法在树生长时引入了额外的随机性；不同于在节点分裂时寻找最好的特征（详见第六章），它在特征集的随机子集中找最好的特征。它导致了树的差异性，并且再一次用高偏差换低方差，总的来说是一个更好的模型。以下的`BaggingClassifier`大致相当于之前的`randomforestclassifier`：
